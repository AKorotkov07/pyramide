...
public class Tree {
    ...
    public boolean isNamePyramid() {
        return isNamePyramidHelper(this);
    }

    private boolean isNamePyramidHelper(Tree node) {
        if (node == null) {
            return true;
        }

        boolean leftValid = (node.left == null) || (node.name.length() <= node.left.name.length() && isNamePyramidHelper(node.left));
        boolean rightValid = (node.right == null) || (node.name.length() <= node.right.name.length() && isNamePyramidHelper(node.right));
